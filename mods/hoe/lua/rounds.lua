...
default_props=
{
	timestep=100, -- one tick every 100 seconds.
...
--	p.endtime=H.srv.time+(p.timestep*4032) -- default game end after about a month of standard ticks
	...
	max_energy=1000,	-- maximum amount of energy a player can have at once
...
local H=srv.H
	...
	if c.endtime == -1 then -- initial patchup
		c.endtime=c.created+(c.timestep*864*14) -- 14days at 100secs per tick (864 energy per day)
	end
