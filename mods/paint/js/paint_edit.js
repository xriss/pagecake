...
var msg_hook=function(msg,dat)
{
	...
	if(msg.cmd=="fat")
	{
		image_src_fat="data:image/gif;base64,"+dat;
...
window.paint_configure=function(lson) {
	var t=window.gamecake.post(
		...
		' win.js_post("cmd=pix\\n",s)\n'+
		' local grd=images.get().export_grd(".GIF").g\n'+
		' local s=mime.b64( (grd:save({fmt="gif"})) )\n'+
		' win.js_post("cmd=fat\\n",s)\n'+
...
//	$("#img_fat").hide();
	window.gamecake=gamecake_loader({div:"#paint_draw",cakefile:head.fs.swankypaint_cake,msg_hook:msg_hook,loaded_hook:loaded_hook});
